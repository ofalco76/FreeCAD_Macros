# PATCHED VERSION — apply to:
#   .venv/Lib/site-packages/ansys/dyna/core/lib/transforms/utils/define_transformation.py
#
# FIX: _get_transform_matrix used post-multiply (mtx @ row_mtx) which reversed
# the DEFINE_TRANSFORMATION row application order. Per LS-DYNA manual Vol.I R11
# §15-319 (Table 15-56):
#   "the transformation matrix defined by MIRROR, SCALE, ROTATE or TRANSL is
#    applied to the previously defined existing matrix to form the new global
#    transformation matrix"
# i.e. M_new = T_row @ M_existing  (pre-multiply).
# Now changed to: mtx = tfm.concatenate_matrices(row_mtx, mtx)
#
...
def _get_transform_matrix(transforms: pd.DataFrame) -> np.ndarray:
    # LS-DYNA manual Vol.I R11 §15-319 (Table 15-56):
    #   "the transformation matrix defined by [each row option] is applied
    #    to the previously defined existing matrix to form the new global
    #    transformation matrix"
    #
    # This means: M_new = T_row @ M_existing  (pre-multiply).
    #
    # With column-vector convention (np.dot(M, p)):
    #   Rows listed top-to-bottom as T1, T2, ..., Tn produce
    #       M_final = Tn @ ... @ T2 @ T1
    #   so a point P is transformed as: M_final @ P = Tn(...(T2(T1(P))))
    #   → T1 is applied first (top row), Tn last (bottom row), all in
    #     GLOBAL coordinates.
    #
    # concatenate_matrices(A, B) = A @ B  (verified against transformations lib).
    # Therefore each new row must PRE-MULTIPLY the accumulated matrix:
    #   mtx = concatenate_matrices(row_mtx, mtx) = row_mtx @ mtx
    #
    # The previous code used POST-multiply (mtx @ row_mtx) which reversed the
    # application order, making parts appear as if transformed in local axes.
    mtx = tfm.identity_matrix()
    for index, transform in transforms.iterrows():
        row_mtx = _get_row_transform_matrix(transform)
        if row_mtx is None:
            return None
        mtx = tfm.concatenate_matrices(row_mtx, mtx)  # pre-multiply → global-axis order
    return mtx
